...
- **Zernio webhook:** `POST /api/webhooks/zernio` receives post lifecycle events and syncs status to Airtable. Auth middleware bypassed for `/api/webhooks` path. Currently registered via ngrok; permanent URL tracked in #67. Note: Zernio does not emit a webhook for "reverted to draft" — only `post.published`, `post.failed`, `post.partial`, `post.scheduled`.
